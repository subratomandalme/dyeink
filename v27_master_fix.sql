...
declare
  ...
  graph_data json;
begin
  -- Totals (Explicit table aliases)
  select coalesce(sum(views), 0) into total_views 
  from public.posts 
  where posts.user_id = get_dashboard_stats.p_user_id;

  select coalesce(sum(shares), 0) into total_shares 
  from public.posts 
  where posts.user_id = get_dashboard_stats.p_user_id;

  -- Subscribers (Explicit aliases)
  ...
  daily_agg as (
      ...
      where p.user_id = get_dashboard_stats.p_user_id
...
-- 2. FIX SITE SETTINGS RLS ERROR (Allow Users to Create/Edit their own settings)
ALTER TABLE public.site_settings ENABLE ROW LEVEL SECURITY;

-- Policy for INSERT (Creation)
DROP POLICY IF EXISTS "Users can insert their own settings" ON public.site_settings;
CREATE POLICY "Users can insert their own settings" ON public.site_settings
...
CREATE POLICY "Users can update their own settings" ON public.site_settings
FOR UPDATE USING (auth.uid() = user_id);

-- 3. ENSURE PERMISSIONS FOR RPCS (Fixes "Access Denied" on incognito)

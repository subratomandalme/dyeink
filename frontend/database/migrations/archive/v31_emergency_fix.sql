-- v31 EMERGENCY FIX
-- USE THIS TO FIX "FUNCTION NOT FOUND" AND "RLS POLICY"ERRORS
...
-- 2. RECREATE FUNCTIONS (Unambiguous Params)
...
-- 3. FIX RLS (Assuming user_id is sent correctly now)
ALTER TABLE public.site_settings ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Enable all access for users based on user_id" ON public.site_settings;
CREATE POLICY "Enable all access for users based on user_id" ON public.site_settings
FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- Also allow public read (for blog visitors)

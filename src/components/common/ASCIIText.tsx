...
(Math as MathExtended).map = function (n: number, start: number, stop: number, start2: number, stop2: number): number {
...
class CanvAscii {
    ...
    updateRotation() {
        const x = (Math as MathExtended).map(this.mouse.y, 0, this.height, 0.5, -0.5);
        const y = (Math as MathExtended).map(this.mouse.x, 0, this.width, -0.5, 0.5);
...
export default function ASCIIText({
    ...
    return (
        ...
        .ascii-text-container pre {
          ...
          left: 0;
          top: 0;
          background-image: linear-gradient(135deg, #ffffff 0%, #888888 50%, #ffffff 100%);

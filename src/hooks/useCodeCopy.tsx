...
export const useCodeCopy = (ref: React.RefObject<HTMLDivElement>) => {
    useEffect(() => {
        ...
        preTags.forEach((pre) => {
            // Requirement: Only add copy button to code blocks with language specification
            const codeElement = pre.querySelector('code')
            const hasLanguageClass = codeElement && Array.from(codeElement.classList).some(cls => cls.startsWith('language-'))

            // If strict mode is desired, uncomment the check. 
            // However, usually all code blocks should be copyable. 
            // Given "only when its a code of some language", we check for the class.
            if (!hasLanguageClass) {
                return
            }
